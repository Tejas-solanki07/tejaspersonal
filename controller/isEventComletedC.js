...
const Update = async (req, res) => {
  try {
    ...
    const updateData = await EventCompletedModel.updateOne({ _id: UpdateId }, cp);
...
const getAllData = async (req, res) => {
  try {
    ...
    let data = await EventCompletedModel.find().populate("CourseId").populate("Astudent").skip(skip).limit(limit).sort({ _id: -1 });
